...
try {
  ...
  (0, import_assert.default)(/^\d+$/.test(alias), `Expecting alias be a number`);
  const eikServerKey = import_core.default.getInput("eik-server-key");
  ...
  const eikrcPath = import_path.default.join(import_os.default.homedir(), ".eikrc");
  const eikrc = JSON.parse(import_fs.default.readFileSync(eikrcPath, { encoding: "utf-8" }));
  const token = new Map(eikrc.tokens).get(eikConfig.server);
  (0, import_assert.default)(token, `No token found in ${eikrcPath} for server ${eikConfig.server}`);
  import_core.default.setSecret(token);
  const type = eikConfig.type || "package";
  const updateAlias = (0, import_child_process.execSync)(`npx @eik/cli alias ${eikConfig.name} ${version} ${alias} --server ${eikConfig.server} --token ${token} --type ${type}`);
